$(document).ready(function() {
    ...
    function initializeGame () {
        ...
        $('#mainMenu').hide();
        $('#gameStats').show();
        $('#gameStats').html('<h3>Level: ' + level + '</h3><h3>Lives: ' + lives + '</h3>');
    ...
    function movePlayerX() {
        ...
        if (Player.xPos + Player.xVel >= Player.xRadius - 50 && Player.xPos + Player.xVel <= arenaWidth - Player.xRadius) {
    ...
    function movePlayerY() {
        ...
        if (Player.yPos + Player.yVel >= Player.yRadius - 50 && Player.yPos + Player.yVel <= arenaHeight - Player.yRadius) {
    ...
    function calculateShot(xPos, yPos) {
        ...
        if (gameOn()) {
            ...
            var distanceDenominator = Math.sqrt(Math.pow(xPos - Player.xPos, 2) + Math.pow(yPos - Player.yPos, 2))

            shotList[numOfShots] = {

                xPos: Player.xPos + (Player.xRadius + 15) * (xPos - Player.xPos) / distanceDenominator,
                yPos: Player.yPos + (Player.yRadius + 15) * (yPos - Player.yPos) / distanceDenominator,
                xShot: xPos + (Player.xRadius + 15) * (xPos - Player.xPos) / distanceDenominator,
                yShot: yPos + (Player.yRadius + 15) * (yPos - Player.yPos) / distanceDenominator,
            ...
            $('#shotLayer').append('<div id="' + index + '" class="playerShot"></div>');
    ...
    function manageBullets() {
        ...
        for (var i in shotList) {
            ...
            playerDistance = Math.pow(((Player.xPos - shot.xPos) / (Player.xRadius + shotRadius)), 2) + Math.pow(((Player.yPos - shot.yPos) / (Player.yRadius + shotRadius)), 2)
            ...
            if (playerDistance <= 0.75) {
                ...
                if (self.lives > 1) {
                    ...
                    $('#gameStats').html('<h3>Level: ' + level + '</h3><h3>Life: ' + lives +'</h3>');
    ...
    function spawnNewEnemies(num) {
        ...
        for (i = 0; i < num; i++) {
            ...
            spawnEnemy((Math.random() - 0.5) * (arenaWidth - 100) + arenaWidth / 2, (Math.random() - 0.5) * (arenaHeight - 100) + arenaHeight / 2);
    ...
    function spawnEnemy(xPos, yPos) {
        ...
        $('#objectLayer').append('<div id="' + index + '" class="enemy" draggable="false"></div>');

        $('#' + index).css('background-color', '#0FFFFF');
        $('#' + index).css('height', 50);
        $('#' + index).css('width', 50);
    ...
    function moveEnemy() {
    	...
    	$('#objectLayer').children().each(function() {
    ...
    function enemyShoot() {
        ...
        $('#objectLayer').children().each(function() {
            ...
            var distanceDenominator = Math.sqrt(Math.pow(enemy.xPos - Player.xPos, 2) + Math.pow(enemy.yPos - Player.yPos, 2))

            shotList[numOfShots] = {

                xPos: enemy.xPos + (enemyXRadius + 15) * (Player.xPos - enemy.xPos) / distanceDenominator,
                yPos: enemy.yPos + (enemyYRadius + 15) * (Player.yPos - enemy.yPos) / distanceDenominator,
                xShot: Player.xPos,
                yShot: Player.yPos,         
            ...
            $('#shotLayer').append('<div id="' + index + '" class="enemyShot"></div>');
            $('#' + index).css({'left': (shotList[index].xPos - shotRadius)+'px', 'top': (shotList[index].yPos - shotRadius)+'px'});
    ...
    function gameOver() {
        ...
        $('#shotLayer').children().each(function() {
            $(this).remove();
        });
        ...
        $("#postGameMessage").show().delay(3000).fadeOut(1000).queue(function(n) {
